                                ...
                                <form class="row g-2">
                                    ...
                                    <div class="col-auto">
                                        <a class="btn btn-primary" href="{{ route('site.create') }}">
                                            <i class="fa-solid fa-plus me-1"></i> Add Sites
                                        </a>
                                    </div><!--end col-->
                                </form>    
                            </div><!--end col-->
                                    ...
                                    @php($i = 1)
                                        @foreach ( $rows as $row )
                                            <tr>
                                                ...
                                                <td>{{ $row['telegram'] }}</td>
                                                <td>{{ $row['youtube'] }}</td>
                                                <td><img src="{{ asset($row->logo) }}" width="100px" alt=""></td>
                                                <td class="text-end">                                                       
                                                    <a href="{{ route('site.edit', $row -> id) }}"><i class="las la-pen text-secondary fs-18"></i></a>
                                                    <a href="{{ route('site.delete', $row -> id) }}"><i class="las la-trash-alt text-secondary fs-18"></i></a>
                                                </td>
                                            </tr>
                                        @endforeach
                                    @endif
